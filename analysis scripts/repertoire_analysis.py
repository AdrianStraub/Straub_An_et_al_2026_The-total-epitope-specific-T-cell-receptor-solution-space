# repertoire_analysis.py
#
# Ribbon plot of the most abundant VDJ combinations (plt_vdj_usage_sc).
...
from scirpy.util import _normalize_counts, _is_na
from scirpy.pl.styling import _init_ax
from scirpy.io import AirrCell


# Load or define the color_dict globally (path can be adjusted if needed)
import os as _os
_color_csv = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)),
                          "..", "10X_mouse_vdj", "vdj_gene_colors.csv")
color_df = pd.read_csv(_color_csv)
default_color_dict = dict(zip(color_df["Gene"], color_df["Color"]))
...
def plt_vdj_usage_sc(
    df: pd.DataFrame,
    *,
    vdj_cols: Sequence = ("va", "ja", "vb", "jb"),
    ...
    color_dict: Union[dict, None] = None,  # default None, set inside function
) -> plt.Axes:
    ...
    df = df.assign(
        cell_weights=_normalize_counts(df, normalize_to)
    ...
    for col_idx, col_name in enumerate(vdj_cols):
        ...
        for i, (segment_size, gene) in list(enumerate(zip(segment_sizes, genes)))[
            ...
            if draw_bars:
                gene_color = (
                    color_dict.get(gene, "lightgrey") if color_dict else "lightgrey"
                    if full_combination else None
                    )
                bar_colors = ax.bar(
                    col_idx + 1,
                    segment_size,
        ...
        for _, ribbon in gene_combinations.iloc[0:max_ribbons, :].iterrows():
            ...
            if full_combination:
                ...
                try:
                    ...
                    ribbon_color = (
                        color_dict.get(tmp_gene)
                        if color_dict and tmp_gene in color_dict
                        else gene_colors[tmp_col].get(tmp_gene, "lightgrey")
                    )
                except KeyError:
